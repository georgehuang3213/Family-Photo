...
import { X, FolderPlus, Check, Edit3 } from 'lucide-react';
import confetti from 'canvas-confetti';

const CATEGORIES = ['家族慶典', '旅遊度假', '溫馨聚會', '節日紀念', '成長歷程', '日常紀錄'];
const EMOJIS = ['🎂', '🏖️', '⛺', '🧧', '🎄', '🚗', '✈️', '👶', '🎓', '📸', '🏠', '🎉'];

export default function CreateAlbumModal({ albumToEdit, onClose, onCreateAlbum, onUpdateAlbum }) {
  const isEditing = !!albumToEdit;

  // Extract initial title without leading emoji if editing
  const getInitialTitle = () => {
    if (!albumToEdit?.title) return '';
    return albumToEdit.title.replace(/^[\s\S]{1,3}\s/, (match) => {
      return EMOJIS.some(e => match.includes(e)) ? '' : match;
    }).trim();
  };

  const getInitialEmoji = () => {
    if (!albumToEdit?.title) return '📸';
    const found = EMOJIS.find(e => albumToEdit.title.includes(e));
    return found || '📸';
  };

  const [title, setTitle] = useState(getInitialTitle);
  const [category, setCategory] = useState(albumToEdit?.category || CATEGORIES[0]);
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditing) {
      ...
      if (onCreateAlbum) onCreateAlbum(newAlbum);
    }
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '42px', height: '42px', borderRadius: '12px', background: 'var(--gradient-main)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {isEditing ? <Edit3 size={20} color="#fff" /> : <FolderPlus size={20} color="#fff" />}
            </div>
            <div>
              <h3 style={{ fontSize: '1.2rem', fontWeight: '700' }}>{isEditing ? '編輯相簿資訊' : '建立新家族相簿'}</h3>
              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{isEditing ? '修改相簿名稱、分類與簡介描述' : '整理分類屬於家族的共同回憶'}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
            ...
            <button type="submit" className="btn btn-primary">
              <Check size={16} /> {isEditing ? '儲存修改' : '建立相簿'}
            </button>
          </div>
